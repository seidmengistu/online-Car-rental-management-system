...
@section('content')
                    ...
                    <div class="col-md-5">
                        ...
                        <div class="search-box">
                            ...
                            <input type="text" name="search" class="form-control-modern"
                                placeholder="Customer name, car, reference..." value="{{ request('search') }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        ...
                        ($reservation->status == 'pending' ? 'warning' :
                            ($reservation->status == 'cancelled' ? 'danger' : 'secondary')) 
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span
                                            class="modern-badge modern-badge-{{ $paymentBadge['class'] }}">{{ $paymentBadge['text'] }}</span>
                                            ...
                                            <div class="d-flex gap-1 mt-2">
                                                ...
                                                <form action="{{ route('admin.reservations.payments.approve', $reservation) }}"
                                                    ...
                                                    <button type="submit" class="btn-icon btn-icon-success"
                                                        style="width: 28px; height: 28px;" title="Approve">
                                                ...
                                                <form action="{{ route('admin.reservations.payments.reset', $reservation) }}" method="POST">
                                                    ...
                                                    <button type="submit" class="btn-icon btn-icon-danger"
                                                        style="width: 28px; height: 28px;" title="Reset">
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            <button class="btn-modern btn-modern-secondary btn-modern-sm dropdown-toggle" type="button"
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end dropdown-menu-modern">
                                                ...
                                                @if($reservation->status == 'confirmed' && $reservation->canBeConvertedToRental())
                                                    <li>
                                                        <form action="{{ route('admin.reservations.convert-to-rental', $reservation) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('PATCH')
                                                            <button type="submit" class="dropdown-item">
                                                ...
                                                @if($reservation->status == 'confirmed' || $reservation->status == 'active')
                                                    ...
                                                    <li>
                                                        <button type="button" class="dropdown-item"
                                                            onclick="openAssignDriverModal('{{ $reservation->id }}', '{{ $reservation->driver_id }}')">
                                                            <i class="bi bi-person-badge"></i>
                                                            {{ $reservation->driver_id ? 'Re-assign Driver' : 'Assign Driver' }}
                                                        </button>
                                                    </li>
                                                @endif
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select name="driver_id" id="driver_id" class="form-select" required>
                                ...
                                @foreach(\App\Models\Driver::where('status', 'active')->orWhere('status', 'on_trip')->get() as $driver)
                            ...
                            <div class="form-text">Note: Assigning a driver will notify the customer.</div>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            function openAssignDriverModal(reservationId, currentDriverId) {
                const form = document.getElementById('assignDriverForm');
                form.action = `/admin/reservations/${reservationId}/assign-driver`;
