            ...
            <form class="row g-3" method="GET" action="{{ route('admin.returns.index') }}">
                <div class="col-md-4">
                    ...
                    <div class="search-box">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" class="form-control-modern"
                            placeholder="Customer, email, plate...">
                    </div>
                </div>
                <div class="col-md-3">
                    ...
                    <select name="verification" class="form-control-modern">
                        ...
                        <option value="pending" {{ request('verification') == 'pending' ? 'selected' : '' }}>Pending Approval
                        </option>
                        <option value="approved" {{ request('verification') == 'approved' ? 'selected' : '' }}>Approved
                        </option>
                    </select>
                </div>
                <div class="col-md-3">
                    ...
                    <select name="overdue_status" class="form-control-modern">
                        <option value="">All</option>
                        <option value="pending" {{ request('overdue_status') == 'pending' ? 'selected' : '' }}>Pending
                        </option>
                        <option value="paid" {{ request('overdue_status') == 'paid' ? 'selected' : '' }}>Paid</option>
                        <option value="not_required" {{ request('overdue_status') == 'not_required' ? 'selected' : '' }}>Not
                            Required</option>
                    </select>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="small">
                                                ...
                                                <div class="fw-semibold mt-1">
                                                    ...
                                                    Actual:
                                                    {{ $rental->actual_end_date ? $rental->actual_end_date->format('M d, Y H:i') : '—' }}
                                                </div>
                                                <div class="mt-2">
                                        ...
                                        <td>
                                            <div class="overdue-info">
                                                <div class="fw-semibold" style="font-size: 18px;">ETB
                                                    {{ number_format($rental->overdue_fee, 2) }}</div>
                            ...
                            default => 'secondary'
                                                ...
                                                @if($rental->overdue_fee > 0)
                                                    <div class="small text-muted mt-2">
                                                        <div>Method:
                                                            {{ $rental->overdue_payment_method ? ucfirst(str_replace('_', ' ', $rental->overdue_payment_method)) : '—' }}
                                                        </div>
                                        ...
                                        <td class="text-end">
                                            <div class="d-flex flex-column gap-2 align-items-end">
                                                <!-- <a href="{{ route('rentals.show', $rental) }}" class="btn-icon btn-icon-primary"
                                                    title="View Details">
                                                    <i class="bi bi-eye"></i>
                                                </a> -->

                                                @if($rental->overdue_receipt_path)
                                                    <a href="{{ asset('storage/' . $rental->overdue_receipt_path) }}" target="_blank"
                                                        class="btn-icon btn-icon-secondary" title="View Receipt">
                                                ...
                                                @if($rental->overdue_fee > 0 && $rental->overdue_payment_status === 'pending' && !$customerProvidedProof)
                                                    <div class="payment-form p-3 rounded" style="background: #f9fafb; width: 200px;">
                                                        <form method="POST" action="{{ route('admin.rentals.overdue.pay', $rental) }}"
                                                            enctype="multipart/form-data">
                                                            @csrf
                                                            @method('PATCH')
                                                            <input type="text" name="overdue_payment_reference" class="form-control-modern mb-2"
                                                                style="padding: 8px; font-size: 12px;" placeholder="Reference #" required>
                                                            <select name="overdue_payment_method" class="form-control-modern mb-2"
                                                                style="padding: 8px; font-size: 12px;" required>
                                                                <option value="cash">Cash</option>
                                                                <option value="telebirr">Telebirr</option>
                                                                <option value="cbe">CBE</option>
                                                                <option value="bank">Bank</option>
                                                            </select>
                                                            <input type="file" name="overdue_receipt_file" class="form-control-modern mb-2"
                                                                style="padding: 4px; font-size: 11px;">
                                                            <input type="text" name="overdue_payment_notes" class="form-control-modern mb-2"
                                                                style="padding: 8px; font-size: 12px;" placeholder="Notes (optional)">
                                                    ...
                                                    @if($canApprove)
                                                        <div class="approve-form p-3 rounded" style="background: #d1fae5; width: 200px;">
                                                            <form method="POST" action="{{ route('admin.returns.approve', $rental) }}">
                                                                @csrf
                                                                @method('PATCH')
                                                                <input type="text" name="return_verification_notes" class="form-control-modern mb-2"
                                                                    style="padding: 8px; font-size: 12px;" placeholder="Verification notes">
                                                                <button type="submit" class="btn-modern btn-modern-success btn-modern-sm w-100">
